...
mydict = ['BanPhuc', 'BanThanh', 'BanVu', 'Quyen', 'ThayDuc', 'Van']
...
def visualize(input, faces, names, fps, thickness=2):
    if faces[1] is not None:
        for idx, face in enumerate(faces[1]):
            ...
            text_size, _ = cv.getTextSize(name, cv.FONT_HERSHEY_SIMPLEX, 0.5, 2)
            text_x = coords[0] + (coords[2] - text_size[0]) // 2
            text_y = coords[1] - 10
            cv.putText(input, name, (text_x, text_y), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
